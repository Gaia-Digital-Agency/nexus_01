# Action Summary — What We Found, and What We're Going to Do

**Updated:** 12 June 2026 · **You are here:** Plan → **Action Summary** → Todo
**In the app:** this is the *Reports + Proposals* layer — the portfolio briefing and the changes we're recommending for approval. Live app: **https://nexus.gaiada.online**
**Built from:** every `audits/<site>.md` (site health) + `seo/<site>.md` (search). **Feeds:** `todo.md` (the task list).

> **See it live:** the numbers behind this summary are on the dashboard at **https://nexus.gaiada.online** — *Reports* mirrors these findings, *Proposals* holds the changes below, *Deployments* tracks the work in `todo.md`.

> **The whole plan in three sentences.** We checked all ~63 sites; most get little traffic, ~7 earn almost all of it. So we'll spend cheaply on security and crawlability fixes everywhere, and put our real SEO effort only on the few sites that can move revenue. One more opportunity (Google Ads, Business Profiles, social) is ready but blocked on a single Google permission — we should start unblocking it now.

---

## The reality we're working with

- **~63 live sites. Only ~7 earn real organic traffic** — aperitif, blossomsteakhouse, cascadesbali, nusapenida, pinstripebar, sepedamotor, viceroybali. The rest are near-zero in search.
...
- **Coverage is complete** — every live site has both a site-health audit and an SEO analysis:

| Server | Sites | How we looked |
|---|---|---|
| gda-ce01 | 17 | full server access — plugin/config-level depth |
| gda-pn01 | 5 | server access — Node.js apps (not WordPress) |
| hostinger | 41 | site responses + Hostinger API (no file-level access) |
| **Total** | **63** | ~54 live · ~7 redirect-only · ~2 parked |

*Everything below is **analysis** — nothing has been changed on a live site yet. Each finding leads with what it means for the business, with the technical term kept in (parentheses) so the engineer doing the fix knows exactly what's meant.*

---

## What we found — Site Health *(is the site safe, and can Google read it?)*
...
## What we found — Search *(can the site win the rankings it should?)*

- **Most sites barely show up in search.** Of 63 sites, only ~7 have a real organic footprint (the earners named above). The rest are near-zero in Semrush. *This is the single most important fact for the plan: concentrate effort, don't spread it.*
...
Work through the Site-Health findings above, worst-first: rotate the shared login keys, kill the `admin` login, close the debug leak, then the portfolio-wide quick wins (add HSTS, stop blocking Google, hide the staging site, retire default `wp_` prefixes, de-duplicate fighting plugins, patch the out-of-date sites). *Why it leads: it protects the business, it's quick, and it clears the ground so SEO work actually gets indexed.*

### Stream 2 — SEO (concentrated on the earners)
- **Fix the titles first — highest return.** It directly addresses the "clicks left on the table" finding: rewriting weak page titles converts traffic we *already* rank for. **viceroybali (8 rewrites) and aperitif (10) are drafted and shippable now.**
- **Then build content** on the Tier-1 sites (full scope, tiers and timelines in `seo/seo-work-scope-20-sites.md`).
- **Defend nailsalonubud's #1** with supporting content before a competitor takes it.
